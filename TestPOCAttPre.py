...
if st.session_state.nav == "My Account":
    ...
    # Test Mode UI
    st.header("Test Mode")
    # Let the user select which test mode they wish to use.
...
*Note: The global settings defined in Train Mode (for Average Employee Age, Women % etc.) will be applied automatically to all bulk data.*
    ...
    selected_test_industry = st.selectbox("Select Your Industry", industry_options,
                                            index=industry_options.index(default_test_industry)
                                            if default_test_industry in industry_options else 0,
                                            key="test_industry")
    ...
    if test_mode_option == "Single Employee":
        ...
        st.write("### Bulk Employee Attrition Prediction")
