...
def update_aggregated_training_data(industry, new_data_df):
    # Replace empty strings or whitespace-only with NaN and then fill with 0 in new data
    new_data_df = new_data_df.replace(r'^\s*$', np.nan, regex=True).fillna(0)
    
    filename = f"training_data_{industry}.csv"
    if os.path.exists(filename):
        try:
            existing_df = pd.read_csv(filename)
            # Clean the existing aggregated data as well:
            existing_df = existing_df.replace(r'^\s*$', np.nan, regex=True).fillna(0)
        except Exception as e:
...
def train_model(training_df, target_column, industry):
    ...
    # Clean the entire training dataframe:
    training_df = training_df.replace(r'^\s*$', np.nan, regex=True).fillna(0)
    ...
    # Log missing values per column (for debugging)
    st.write("Missing values per column in training data after encoding:", X_encoded.isnull().sum())
